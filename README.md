...
实际到位后以 `100%` 占空比点亮紫外灯。转轮运动期间，右侧曲线区域显示
MF500 USB 摄像头实时画面，电压采样继续在后台运行。
...
开发调试时也可使用外接键盘的 `Esc` 退出。未分配动作的矩阵键仍会显示在
顶部“最近按键”区域，但不会改变设备状态。
...
摄像头在线程中持续保留最新帧，不阻塞 pygame、电机或 ADS1256。转轮运动
时右侧曲线区显示实时画面，电机到位或运动失败后恢复曲线。
