...
UI 建立 HDMI/SDL 显示后会自动播放开机自检，再进入实验主界面。硬件模式按
以下顺序逐项显示“等待、检查中、正常或异常”，单项失败不会中断后续检查：

1. HDMI 显示：确认显示表面可用且分辨率为 `1024x600`。
2. ADS1256：检查 SPI、DRDY、寄存器回读、自校准和首批采样。
...
存在异常时显示完整设备列表和原因 2.5 秒。之后仍自动进入主界面，但失败的
设备使用不可用占位驱动，对应的测量、转轮、灯光或摄像功能会被禁用，
也不会使用模拟数据冒充硬件。终端和 systemd 日志同步输出 `[SELF-TEST]` 记录。
...
| -------: | --: | ---------------------- | ----------------: | :---------------- | ------------------ | --: | -------: |
|        - |   - | -                      |         1（3.3V） | 2（5V）           | -                  |   - |        - |
|        8 |   2 | 400 nm 紫外灯 PWM 输出 |   3（GPIO2/SDA1） | 4（5V）           | -                  |   - |        - |
|        9 |   3 | 450 nm 蓝灯 PWM 输出   |   5（GPIO3/SCL1） | 6（GND）          | -                  |   - |        - |
|        7 |   4 | 520 nm 绿灯 PWM 输出   |        7（GPIO4） | 8（GPIO14/TXD）   | EMM TXD -> 驱动 RX |  14 |       15 |
|        - |   - | -                      |          9（GND） | 10（GPIO15/RXD）  | EMM RXD <- 驱动 TX |  15 |       16 |
|        0 |  17 | LED4 PWM 输出          |      11（GPIO17） | 12（GPIO18）      | -                  |  18 |        1 |
|        2 |  27 | LED5 PWM 输出          |      13（GPIO27） | 14（GND）         | -                  |   - |        - |
|        3 |  22 | LED6 PWM 输出          |      15（GPIO22） | 16（GPIO23）      | ADS1256 SCLK 输出  |  23 |        4 |
...
- BCM2/3（物理 3/5）是 I2C1 SDA1/SCL1 且通常带上拉，当前分别用于
  400 nm 紫外灯和 450 nm 蓝灯 PWM；使用这两路时不能同时启用 I2C1。
- BCM4（物理 7）当前用于 520 nm 绿灯 PWM。
- BCM17/27/22（物理 11/13/15）分别用于 LED4/LED5/LED6 PWM。
...
`D0-D3` 是 ADS1256 数字 I/O，不是模拟输入；本项目不连接且程序不申请这
四个引脚。原先占用的 BCM17/18/27/22 已释放，其中 BCM17/27/22 改作
LED4/LED5/LED6，BCM18 保持未分配。模拟输入仍为 `AIN0-AIN7`。
...
工具依次检查 SPI GPIO、DRDY、前 5 个寄存器和 DRATE 写入回读，不会申请或
驱动 D0-D3。寄存器全 `0xFF` 时优先检查 `CS/DOUT/GND`；全 `0x00` 时检查供电、
...
中间灯位聚焦时按 `A` 进入左栏手动角度模块：数字键输入当前灯位绝对角度，
`*` 输入小数点，允许范围为 `0~360°`。输入数字时电机不会转动；按 `A`
...
`LED/led_pwm.py` 驱动以下六路输出，默认输出 1 kHz 主动高电平 PWM，UI
初始占空比为 `100%`：

| 灯位 | 波长与颜色    | WiringPi | BCM | 物理脚 |
| ---: | ------------- | -------: | --: | -----: |
|    0 | 400 nm 紫外灯 |        8 |   2 |      3 |
|    1 | 450 nm 蓝灯   |        9 |   3 |      5 |
|    2 | 520 nm 绿灯   |        7 |   4 |      7 |
...
GPIO 不能直接给灯供电，必须连接 MOSFET、三极管或带 PWM 输入的恒流
驱动器，并与树莓派共地。BCM2/3 通常带上拉，外部驱动级必须保证程序接管前
默认关断；六路均为低电平有效模块时，启动 UI 增加 `--led-active-low`。

对应灯仅在以下条件全部满足时输出 PWM：转轮实际到达对应灯位且静止、测量
状态为“测量中”、占空比大于 0。暂停测量、开始转轮、切换灯位或退出程序时
立即熄灭全部六路。
...
该命令默认测试紫外灯；测试蓝灯、绿灯或后三路时分别增加 `--lamp blue`、
`--lamp green`、`--lamp led4`、`--lamp led5` 或 `--lamp led6`。低电平有效时
