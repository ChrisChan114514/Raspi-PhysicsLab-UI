...
硬件模式使用 `23` 个唯一 BCM GPIO：ADS1256 使用 10 个，矩阵键盘使用
8 个，三路灯 PWM 使用 3 个，EMM 电机 GPIO UART 使用 2 个。USB 摄像头
...
`LED/led_pwm.py` 驱动以下三路输出，默认输出 1 kHz 主动高电平 PWM，UI
初始占空比为 `100%`：
...
默认关断；三路均为低电平有效模块时，启动 UI 增加 `--led-active-low`。

对应灯仅在以下条件全部满足时输出 PWM：实际到位灯位为 400 nm 紫外光、
450 nm 蓝光或 520 nm 绿光之一，转轮已到位且静止、测量状态为“测量中”、
占空比大于 0。暂停测量、开始转轮、切换灯位或退出程序时立即熄灭全部三路。
...
该命令默认测试紫外灯；测试蓝灯或绿灯时分别增加 `--lamp blue` 或
`--lamp green`。低电平有效时增加 `--active-low`。正常结束或 `Ctrl+C`
中断都会恢复熄灯电平。
...
LED/led_pwm.py                  400/450/520 nm 三路灯 PWM 驱动
